...
You can set "Folder name" to "DokomoDev" to use Dokomo's development instance instead:

pnpm create WhatsApp DokomoDev
`);
...
(async () => {
  ...
  if (!fs.existsSync(recipesFolder)) {
    console.log(
      `Couldn't find your recipe folder (${recipesFolder}). Is Dokomo installed?`,
    );
    return;
  }
